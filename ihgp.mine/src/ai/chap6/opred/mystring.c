	...
	string(s), delimiter(d)
{
	...
	for (int match = 1; match && tokenStart < stringLength; )
	{
		match = 0;
		for (int id = 0; !match && id < delimiterLength; id++)
		{
			if (string[tokenStart] == delimiter[id])
				match = 1;
		}
		if (match) tokenStart++;
	}
	...
	// find end of token
	match = 0;
	for (tokenEnd = tokenStart; !match && tokenEnd < stringLength; )
	{
		for (int id = 0; !match && id < delimiterLength; id++)
		{
			if (string[tokenEnd] == delimiter[id])
				match = 1;
		}
		if (!match) tokenEnd++;
	}
...
{
	return(tokenEnd > stringLength-1);
}
