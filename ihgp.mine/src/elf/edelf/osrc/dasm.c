...
pdasm(long addr, long size)
{
	...
	if (pehdr->e_machine != EM_SPARC && pehdr->e_machine != EM_PPC)
	{
		printf("Not a SPARC or PPC file; disassembler is only for SPARC and PPC.\n");
		return;
	}
	if ((pphdrs[addrseg].p_flags & PF_X) == 0)
	{
	...
	if (size > pphdrs[addrseg].p_filesz-start)
	{
		size = pphdrs[addrseg].p_filesz-start;
	}
...
sdasm(long offset, long size)
{
	...
	if (pshdrs[offsetsec].sh_type != SHT_PROGBITS)
	{
	...
	if (size > pshdrs[offsetsec].sh_size-start)
	{
		size = pshdrs[offsetsec].sh_size-start;
	}
	printf("section %d: %s (%ld, %ld)\n", offsetsec, 
		pshstrtbl+pshdrs[offsetsec].sh_name,
		pshdrs[offsetsec].sh_offset, 
		pshdrs[offsetsec].sh_size);
	switch (pehdr->e_machine)
	{
	case EM_SPARC:
...
editdasm(char *)
{
	...
	for (int done=0; !done; )
	{
		...
		else if (strcmp(pt, "lf") == 0)
		{
			...
			// scan for functions
			Elf32_Sym *psym = NULL;
			for (int csec=0; csec<pehdr->e_shnum;csec++)
			{
				     ...
				     csymno++)
				{
					...
					if (pfcn != NULL && *pfcn != '\0' &&
					    *pfcn != '*' &&
						...
						continue;
					showsymbolname(csymno, csec, p0, pe);
				}
			}
		}
		...
		else if (strcmp(pt, "d") == 0)
		{
			...
			else
			{
				...
				// file offsets.
				offset = pshdrs[psym->st_shndx].sh_offset;
