...
protected:
	...
	//
	// the index is built by traversing the tree and counting
	// bits. the path length gives the number of bits stored
	// in the index. 
	void dumpbits(int nbits, unsigned short data, const char *msg) {
		// char *buf = new char [nbits+1];
		char buf[BUFSIZ];
		buf[nbits] = '\0';
	...
	void buildIndex(HuffmanNode *pn, int pathlen, unsigned short uscode) {
DUMP(pathlen);
DUMP((unsigned long)uscode);
dumpbits(pathlen, uscode, "uscode: ");
		// check if we are the end of the current path
		if (pathlen >= hidxkeysz || pn->isALeaf())
		{
DUMP(hindex[uscode].inuse);
			MustBeTrue(!hindex[uscode].inuse);
			hindex[uscode].inuse = 1;
			hindex[uscode].nbits = pathlen;
			hindex[uscode].code = uscode;
			hindex[uscode].node = pn;
			return;
		}

		// continue traversing the huffman tree
		buildIndex(pn->getLeft(), pathlen+1, (uscode<<1));
		buildIndex(pn->getRight(), pathlen+1, (uscode<<1)|1);
		return;
	}
	void buildIndex(int maxpathlen) {
DUMP(maxpathlen);
		// since the maximum allowed bits in a Huffman code
		// is the number of bits in an unsigned short (my
		// choice), the maximum path length is the same.
		//
		MustBeTrue(0 < maxpathlen && maxpathlen <= BitsInUShort);
		...
		hidxkeysz = maxpathlen+1;
		hindexsz = ((unsigned short)1)<<(maxpathlen+1);
		hindex = new HuffmanIndex [hindexsz];
		MustBeTrue(hindex != NULL);

		// traverse the huffman tree, filling in the
		// index as it goes.
		unsigned short uscode = 1;
		buildIndex(phroot->getLeft(), 1, (uscode<<1));
		buildIndex(phroot->getRight(), 1, (uscode<<1)|1);

		// dump the index
		for (int i=0; i<hindexsz; i++)
		{
			if (!hindex[i].inuse)
				continue;
			cerr << "i: " << i << endl;;
			cerr << "inuse: " << (int)hindex[i].inuse << endl;;
			cerr << "nbits: " << (int)hindex[i].nbits << endl;;
			cerr << "code: " << (unsigned int)hindex[i].code << endl;;
			cerr << "node: " << (unsigned long)hindex[i].node << endl;;
		}
		return;
	}
