#ident "@(#)/usr/src/head/syms.h	3.1.2.1 12/19/90 16:22:23 - (c) Copyright 1988, 1989, 1990 Amdahl Corporation"
...
#define  P_TV		(unsigned short)-4
	...
	/* indicates symbol is a weak external (not to be searched
	 * for in a library and not to be complain about as
	 * unresolved */
#define N_WUNDEF        -8
...
union auxent
{
	...
	struct
	{
		long	_zeroes;		/* Name in strtab if == 0 */
		long	_offset;		/* offset into string table */
	} _file_offset;
	struct
	{
...
#define x_fzeroes	_file_offset._zeroes
#define x_foffset	_file_offset._offset
