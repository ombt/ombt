...
NQueenNode::heuristic(const NQueenNode &start)
{
	...
	{
		int numberOfSpacesInRow = 0;
		for (int ic = 0; ic < ncols; ic++)
		{
			if (board[ir*nrows+ic] == SpaceChar)
			{
				numberOfSpaces++;
				numberOfSpacesInRow++;
			}
		}
		if (numberOfSpacesInRow == 2)
			numberOfSpaces -= numberOfSpacesInRow - 1;
	}
#else
#ifdef QUEEN2
	for (int ir = 0; ir < nrows; ir++)
	...
	for (int ir = 0; ir < nrows; ir++)
	{
		...
		if (numberOfSpacesInRow == 0)
		{
			numberOfSpaces = INT_MAX/2;
			break;
		}
	}
#else
