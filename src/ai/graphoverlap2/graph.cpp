...
// e: fromid:3 toid:4 color:blue
// e: fromid:4 toid:5 color:blue
...
Graph::clearVisited()
{
    Vertices::iterator vit = vertices_.begin();
    Vertices::iterator vitend = vertices_.end();
    for ( ; vit!=vitend; ++vit)
    {
        vit->second.visited_ = false;
    }
}

void
Graph::connected(Graph::VertexId vid, long &visited, bool clear)
{
    if (clear) clearVisited();
    visited += 1;
    vertices_[vid].visited_ = true;
    ...
    for ( ; tosetit!=tosetitend; ++tosetit)
    {
        ...
        if (!vertices_[tosetvid].visited_)
        {
            connected(tosetvid, visited);
        }
    }
    ...
    for ( ; fromsetit!=fromsetitend; ++fromsetit)
    {
        ...
        if (!vertices_[fromsetvid].visited_)
        {
            connected(fromsetvid, visited);
        }
    }
}

void
Graph::connectedViaTo(Graph::VertexId vid, long &visited, bool clear)
{
    if (clear) clearVisited();
    ...
    for ( ; tosetit!=tosetitend; ++tosetit)
    {
        ...
        {
            visited += 1;
            vertices_[tosetvid].visited_ = true;
            connectedViaTo(tosetvid, visited);
        }
    }
}

void
Graph::connectedViaFrom(Graph::VertexId vid, long &visited, bool clear)
{
    if (clear) clearVisited();
    ...
    for ( ; fromsetit!=fromsetitend; ++fromsetit)
    {
        ...
        {
            visited += 1;
            vertices_[fromsetvid].visited_ = true;
            connectedViaFrom(fromsetvid, visited);
        }
    }
}
