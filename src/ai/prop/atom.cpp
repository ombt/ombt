...
Atom &
Atom::operator=(const ombt::String &rhs)
{
	atom = rhs;
	return(*this);
}
...
Atom::operator~() const
{
	if (atom(0,1) == ombt::String("~"))
		return(Atom(atom(1)));
	else
		return(Atom(ombt::String("~") + atom));
}
...
// input and output
std::ostream &
operator<<(std::ostream &os, const Atom &s)
{
	os << s.atom;
	return(os);
}
...
std::istream &
operator>>(std::istream &is, Atom &s)
{
	is >> s.atom;
	return(is);
}
