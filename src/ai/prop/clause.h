...
public:
	...
	Clause();
	Clause(const Clause &);
	Clause(const ombt::Set_List<Atom> &, int = -1, int = 0);
	virtual ~Clause();
...
public:
	...
	// dump data
	friend std::ostream &operator<<(std::ostream &, const LClause &);
};
...
protected:
	// internal data
	ombt::Set_List_Iterator<Atom> iterator;
};
