...
static bool usetovertices = false;
static bool usefromvertices = false;

struct PQItem1 {
public:
    PQItem1(Graph::VertexId vid = -1, long priority = -1, 
           bool reverse = false):
           vid_(vid), priority_(priority), reverse_(reverse) { }
    PQItem1(const PQItem1 &pqi):
           vid_(pqi.vid_), priority_(pqi.priority_), 
           reverse_(pqi.reverse_) { }
    ~PQItem1() { }
    PQItem1 &operator=(const PQItem1 &pqi) {
        if (this != &pqi) {
            vid_ = pqi.vid_;
    ...
    bool operator<(const PQItem1 &rhs) const {
        if (reverse_)
...
{
    std::cout << "usage: " << cmd << " [-?] [-d] [-t] [-f] graphfile1 graphfile2" << std::endl;
}
...
static void
printpq(const std::string &msg, Graph &g, std::priority_queue<PQItem1> &pq)
{
    std::cout << std::endl;
    ...
    Graph &g1only, Graph &g2only, Graph &g1g2overlap)
{
    ...
    else if (g2visited.find(g2vid) != g2visited.end())
    {
        return;
    }
    ...
    std::cout << "VID: G1:" << g1vid << " G2: " << g2vid << std::endl;
    std::cout << "LABEL: G1:" << g1.label(g1vid) << " G2: " << g2.label(g2vid) << std::endl;
    std::cout << "EDGES: G1:" << g1.getToVertices(g1vid).size() << " G2: " << g2.getToVertices(g2vid).size() << std::endl;
    ...
    {
        std::cout << std::endl;
        std::cout << "MATCH for :" << g1vid << "," << g2vid << std::endl;
    ...
    {
        std::cout << std::endl;
        std::cout << "NO MATCH for :" << g1vid << "," << g2vid << std::endl;
        return;
    }
#else
    std::cout << std::endl;
    ...
    // traverse children
    std::set<Graph::VertexId> g1tovids = g1.getToVertices(g1vid);
    std::set<Graph::VertexId>::const_iterator g1tvit = g1tovids.begin();
    std::set<Graph::VertexId>::const_iterator g1tvitend = g1tovids.end();
    for ( ; g1tvit!=g1tvitend; ++g1tvit)
    {
DUMP2(*g1tvit, g1.label(*g1tvit));
        std::set<Graph::VertexId> g2tovids = g2.getToVertices(g2vid);
        std::set<Graph::VertexId>::const_iterator g2tvit = g2tovids.begin();
        std::set<Graph::VertexId>::const_iterator g2tvitend = g2tovids.end();
        for ( ; g2tvit!=g2tvitend; ++g2tvit)
        {
DUMP2(*g2tvit, g2.label(*g2tvit));
...
graphOverlap(Graph &g1, Graph &g2, 
    Graph &finalg1only, Graph &finalg2only, Graph &finalg1g2overlap)
{
    std::priority_queue<PQItem1> g1pq;
	...
	std::cout << std::endl;
        ...
        {
            long tovisited = 0;
            g1.connectedViaTo(g1vid, tovisited);
            std::cout << "Vertices visited Via To-Links: " << tovisited;
            std::cout << std::endl;
            totalvisited += tovisited;
        }

        if (usefromvertices)
        {
            long fromvisited = 0;
            g1.connectedViaFrom(g1vid, fromvisited);
            std::cout << "Vertices visited Via From-Links: " << fromvisited;
            std::cout << std::endl;
            totalvisited += fromvisited;
        }

        g1pq.push(PQItem1(g1vid, totalvisited));
    }

    std::priority_queue<PQItem1> dummypq;
    dummypq = g1pq;
    printpq("G1 PQ:", g1, dummypq);

    std::priority_queue<PQItem1> g2pq;
	...
	std::cout << std::endl;
        ...
        {
            long tovisited = 0;
            g2.connectedViaTo(g2vid, tovisited);
            std::cout << "Vertices visited Via To-Links: " << tovisited;
            std::cout << std::endl;
            totalvisited += tovisited;
        }
        ...
        {
            long fromvisited = 0;
            g2.connectedViaFrom(g2vid, fromvisited);
            std::cout << "Vertices visited Via From-Links: " << fromvisited;
            std::cout << std::endl;
            totalvisited += fromvisited;
        }

        g2pq.push(PQItem1(g2vid, totalvisited));
    }
...
{
    // check args
    for (int c=0; (c=getopt(argc, argv, "?dtf")) != EOF; )
    {
        switch (c)
        {
        case 'd':
