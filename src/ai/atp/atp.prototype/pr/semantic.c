...
{
	return(getClauses(clist, prep->conclusion, prep->setofsupport));
}
int
Semantic::getClauses(List<Semantic *> &clist, int concluded, int sos)
{
	// check if an expression
	if (!isExpression())
	{
	...
	if (pe->left != NULL && pe->left->getClauses(clist, concluded, sos) != OK)
		return(NOTOK);
	if (pe->right != NULL && pe->right->getClauses(clist, concluded, sos) != OK)
		return(NOTOK);
