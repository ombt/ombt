...
Substitution::applyTo(Literal &l) const
{
	...
	if (l.type != Literal::Function && l.type != Literal::Equal)
	{
		return(OK);
	}
...
Substitution::applyTo(Clause &cl) const
{
	...
	for ( ; !clIter.done(); clIter++)
	{
		...
		{
			ERROR("insert failed.", EINVAL);
			return(NOTOK);
		}
	}
	if (cl.getQuery())
	{
		ClauseAnswersIterator clansIter(cl);
		for ( ; !clansIter.done(); clansIter++)
		{
			Literal literal(clansIter());
			if (applyTo(literal) != OK)
			{
				ERROR("applyTo failed.", EINVAL);
				return(NOTOK);
	...
	newcl.setSOS(cl.getSOS());
	newcl.setQuery(cl.getQuery());
	newcl.setDepth(cl.getDepth());
	newcl.setNumber(cl.getNumber());
	newcl.setConclusion(cl.getConclusion());
	cl = newcl;
