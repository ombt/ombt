...
SemanticRep::SemanticRep(int c, int s, int q):
	conclusion(c), setofsupport(s), query(q)
{
	// nothing to do here
}
...
Semantic::removeExtraNots()
{
	...
	int saveSOS = prep->setofsupport;
	int saveQuery = prep->query;
	...
	prep->setofsupport = saveSOS;
	prep->query = saveQuery;
...
int
Semantic::getClauses(List<Semantic *> &clist, int concluded, int sos, int query)
{
	// check if an expression
	if (!isExpression())
	{
		...
		pnew->prep->setofsupport = sos;
		pnew->prep->query = query;
	...
	if (pe->left != NULL && 
		pe->left->getClauses(clist, concluded, sos, query) != OK)
	{
		ERROR("getClauses failed.", EINVAL);
		return(NOTOK);
	}
	if (pe->right != NULL && 
		pe->right->getClauses(clist, concluded, sos, query) != OK)
	{
		ERROR("getClauses failed.", EINVAL);
		return(NOTOK);
	}
	...
	else if (isTerm())
	{
		...
		if (pa->type == Term::Variable)
		{
			...
			else
			{
				...
				if (updateVariableNames(pa->name, uname) != OK)
				{
					ERROR("update variable names failed.", 
						EINVAL);
					return(NOTOK);
...
int
Semantic::getClausesAsLists(List<String> &clause, String &term, int isnegated)
{
	...
	if (isExpression())
	{
		...
		{
			ERROR("getClausesAsLists failed.", EINVAL);
			return(NOTOK);
		}
	}
	else if (isPredicate())
	{
		    ...
		    (pp->type == Predicate::Equal))
		{
			...
			for ( ; !pargsIter.done(); pargsIter++)
			{
				...
				{
					ERROR("getClausesAsLists failed.", 
						EINVAL);
					return(NOTOK);
				}
			}
	...
	else if (isTerm())
	{
		...
		else if (pa->type == Term::Variable)
		{
			// add variable name
			term += pa->name + String(" ");
		}
		else if (pa->type == Term::Constant)
		{
			// add constant name
			term += pa->name + String(" ");
		}
		else if (pa->type == Term::Number)
		{
			// add number value
			term += pa->value + String(" ");
		}
		else if (pa->type == Term::QuotedString)
		{
			// add quoted string
...
int
Semantic::getClausesAsLists(List<String> &clause)
{
	// check type of record
	if (isExpression())
	{
		...
		if (pe->type == Expression::Or)
		{
			...
			MustBeTrue(pe->left != NULL);
			if (pe->left->getClausesAsLists(clause) != OK)
			{
				ERROR("getClausesAsLists failed.", EINVAL);
				return(NOTOK);
			}
			...
			MustBeTrue(pe->right != NULL);
			if (pe->right->getClausesAsLists(clause) != OK)
			{
				ERROR("getClausesAsLists failed.", EINVAL);
				return(NOTOK);
			}
...
int
Semantic::getClausesAsLists(List<List<String> > &clist)
{
	...
	// set options, put options as a special record before all
	// the literals in the clause. beats having to define a new
	// class with flags.
	//
	String options;
	options += String("{ ");
	if (getConclusion())
		options += String("CONCLUSION ");
	if (getQuery())
		options += String("QUERY ");
	if (getSOS())
		options += String("SET-OF-SUPPORT ");
	options += String("} ");
	clause.insertAtFront(options);
