...
usage(char *cmd)
{
	...
	cerr << "\t-a - value of A in A*tan(K*x+Phi)," << endl;
	cerr << "\t-k - value of K in A*tan(K*x+Phi)," << endl;
	cerr << "\t-p - value of Phi in A*tan(K*x+Phi)," << endl;
	return;
}
...
{
	// variables for generating tangent wave.
	int hdr = 0;
	double deg2rads = 1.0;
	double a = 1.0;
	double k = 1.0;
	...
	{ 
		cout << angle << " " << a*tan((k*angle+phi)*deg2rads) << endl;
	}
