...
public: // data types and constants
    ...
    typedef multimap<Location, Location> AllowedMovesType;
    typedef AllowedMovesType::iterator AllowedMovesTypeIt;
    typedef AllowedMovesType::const_iterator AllowedMovesTypeCIt;
...
protected:
    ...
    AllowedMovesType _zero_vowel;
    AllowedMovesType _one_vowel;
    AllowedMovesType _two_vowel;
};
    ...
    if (_board[r][c]._value != null_key)
    {
        ...
        for (int move=0; el_moves[move][0] != 0; ++move)
        {
            ...
            if (check_if_allowed(vowels, new_r, new_c))
            {
                os << _board[new_r][new_c] << endl;
                moves.insert(
                    AllowedMovesType::value_type(Location(r,c),
