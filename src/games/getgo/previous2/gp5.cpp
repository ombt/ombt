...
public:
    ...
    void exact_one_vowel_moves(ostream &os, int r, int c);
    void exact_zero_vowel_moves(ostream &os, int r, int c);
...
KeyBoard::KeyBoard()
{
    ...
    _exact_one_vowel  = new int [imax];
    for (int i=0; i<imax; ++i)
    {
        _exact_one_vowel[i] = -1;
    }
    _exact_zero_vowel  = new int [imax];
    for (int i=0; i<imax; ++i)
    {
...
void
KeyBoard::exact_one_vowel_moves(ostream &os, int r, int c)
{
   int how_many = 0;
   vowel_moves(os, 1, _one_vowel, r, c, how_many, true);
   os << "Size of exact one-vowel list of allowed moves: " 
      << how_many << endl;
}

void
KeyBoard::exact_zero_vowel_moves(ostream &os, int r, int c)
{
   int how_many = 0;
...
main(int argc, char **argv)
{
    ...
    for (int r=0; r<KeyBoard::max_row; ++r)
    {
        for (int c=0; c<KeyBoard::max_col; ++c)
        {
            ...
            cout << endl << ">>>> generate allowed exact one vowel moves for key: " 
                 << b.key(r, c) << endl;
            b.exact_one_vowel_moves(cout, r, c);

            cout << endl << ">>>> generate allowed exact zero vowel moves for key: " 
                 << b.key(r, c) << endl;
            b.exact_zero_vowel_moves(cout, r, c);
        }
    }
