...
public: // data types and constants
    ...
    typedef vector<Locations> AllowedMoves;
...
KeyBoard::KeyBoard()
{
    ...
    // resize allowed moves lists
    ++_key_num;
    _one_vowel.resize(_key_num);
    _zero_vowel.resize(_key_num);
}
                      ...
                      int c)
{
    ...
    if (_board[r][c]._value != null_key)
    {
        ...
        for (int move=0; el_moves[move][0] != 0; ++move)
        {
            ...
            {
                os << _board[new_r][new_c] << endl;
                moves[index].push_back(Location(new_r, new_c));
            }
        }
    }
}
                          ...
                          long &seq_found)
{
    ...
    if (vowels >= 1)
    {
        int max = _one_vowel[index].size();
        for (int i=0; i<max; ++i)
        {
            int new_r = _one_vowel[index][i].first;
            int new_c = _one_vowel[index][i].second;
            smart_sequences(os, seq, seq_num, vowels, new_r, new_c, verbose, seq_found);
        }
    }
    else
    {
        int max = _zero_vowel[index].size();
        for (int i=0; i<max; ++i)
        {
            int new_r = _zero_vowel[index][i].first;
            int new_c = _zero_vowel[index][i].second;
            smart_sequences(os, seq, seq_num, vowels, new_r, new_c, verbose, seq_found);
        }
    }
}
