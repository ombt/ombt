...
static void
showstab(int &cstabno, int &csec, char *&p0, char *&pe, char &buf, int showlnno, int &multilined)
{
	// init variables
	multilined = 0;
	int length = 0;
	...
	// when more than one file contents are in one object or executable
	// file.
	int showoffset = calcoffset(csec, cstabno);

	// current stab entry
	int cstabno2 = cstabno;
	stab *pstab = (stab *)(p0+cstabno*pshdrs[csec].sh_entsize);
	stab *pstab2 = NULL;
	char *pstabstr = pstrtbls[pshdrs[csec].sh_link]+pstab->n_strx+showoffset;
	char *pstabstr2 = NULL;
	...
	if (isMultilined(pstabstr))
	{
		...
		do {
			...
			pstab2 = (stab *)(p0+cstabno2*pshdrs[csec].sh_entsize);
			pstabstr2 = pstrtbls[pshdrs[csec].sh_link]+
				pstab2->n_strx+calcoffset(csec, cstabno2);
		...
		for (csec=0; csec<pehdr->e_shnum; csec++)
		{
			     ...
			     pt3=NULL, cstabno++)
			{
				...
				if (!REequal(pt3, pstring))
					continue;
				showstab(cstabno, csec, p0, pe, buf, 1, multilined);
				if (buf == 'q') 
					goto all_done;
				char s[BUFSIZ];
			...
			for (csec=0; csec<pehdr->e_shnum; csec++)
			{
				     ...
				     cstabno++)
				{
					...
					if (pstab->n_type != search_type)
						continue;
					showstab(cstabno, csec, p0, pe, buf, 0, multilined);
					if (buf == 'q') 
						goto all_done2;
					char s[BUFSIZ];
					printf("next stab? [n/y/cr=y] ");
