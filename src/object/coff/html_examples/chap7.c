...
// seek or read a symbol table entry
int
lseek2syment(int fd, filehdr &fhdr, int symidx)
{
	// calculate offset to symbol table entry
	unsigned long offset = fhdr.f_symptr + symidx*SYMESZ;

	// seek from start of file
	if (lseek(fd, offset, SEEK_SET) == -1)
	{
		fprintf(stderr, 
		"unable to lseek to symbol [%d], file is %s, errno=%d.\n",
			symidx, currentfile, errno);
		return(NOTOK);
	}
	return(OK);
}
...
int
readsyment(int fd, int symidx, syment &symhdr)
{
	// read symbol table entry
	if (read(fd, (void *)&symhdr, SYMESZ) != SYMESZ)
	{
		fprintf(stderr, 
		"unable to read symbol [%d], file is %s, errno=%d.\n",
			symidx, currentfile, errno);
		return(NOTOK);
	}
	return(OK);
...
// seek or read an auxiliary table entry
int
lseek2auxent(int fd, filehdr &fhdr, int symidx)
{
	// calculate offset to symbol table entry
	unsigned long offset = fhdr.f_symptr + symidx*AUXESZ;

	// seek from start of file
	if (lseek(fd, offset, SEEK_SET) == -1)
	{
		fprintf(stderr, 
		"unable to lseek to auxiliary [%d], file is %s, errno=%d.\n",
			symidx, currentfile, errno);
		return(NOTOK);
	}
	return(OK);
...
int
readauxent(int fd, int symidx, auxent &auxhdr)
{
	// read symbol table entry
	if (read(fd, (void *)&auxhdr, AUXESZ) != AUXESZ)
	{
		fprintf(stderr, 
		"unable to read auxiliary [%d], file is %s, errno=%d.\n",
			symidx, currentfile, errno);
		return(NOTOK);
	}
	return(OK);
...
int
printreloc(int fd, filehdr &fhdr, int isec, scnhdr &shdr)
{
...
/* get a symbol name given an index */
int 
getsymname(int fd, filehdr &fhdr, int symndx, char *string)
{
	// seek to location of symbol entry
	if (lseek2syment(fd, fhdr, symndx) != OK)
		return(NOTOK);

	// read in symbol table entry
	syment symhdr;
	if (readsyment(fd, symndx, symhdr) != OK)
		return(NOTOK);
	...
	if (symhdr._n._n_n._n_zeroes == 0L)
	{
		...
		long offset = fhdr.f_symptr + fhdr.f_nsyms*SYMESZ + 
				symhdr._n._n_n._n_offset;
		if (lseek(fd, offset, SEEK_SET) == -1)
		{
			fprintf(stderr, 
				"unable to lseek file[%s], errno=%d.\n",
				currentfile, errno);
			return(NOTOK);
		}

		// read in  string since the string is NULL terminated
		char *pstr = string;
		for ( ; (read(fd, (void *)pstr, 1) != -1) && 
			*pstr != 0; pstr++) ;
		*pstr = 0;
	}
	else
...
int
printlnno(int fd, int il, filehdr &fhdr, lineno &lnno)
{
	// print data
	if (lnno.l_lnno == 0)
	{
		...
		char string[BUFSIZ];
		if (getsymname(fd, fhdr, lnno.l_addr.l_symndx, string) != OK)
		{
			fprintf(stderr, 
			"unable to read string, file[%s], errno=%d.\n",
				currentfile, errno);
			return(NOTOK);
		}
		fprintf(stdout,"syment[%d]._n._n_name: %s\n",
			lnno.l_addr.l_symndx, string);
	}
	else
	{
...
int
printlnno(int fd, filehdr &fhdr, int isec, scnhdr &shdr)
{
	// check if relocation data
	if (shdr.s_nlnno <= 0)
	{
	...
	for (int il = 1; il <= shdr.s_nlnno; il++)
	{
		...
		// print data
		if (printlnno(fd, il, fhdr, lnno) != OK)
			return(NOTOK);
	}
	return(OK);
}
...
main(int argc, char **argv)
{
	...
	for (int arg = 1; arg < argc; arg++)
	{
		...
		for (int isec = 1; isec <= fhdr.f_nscns; isec++)
		{
			...
			// section
			if (printreloc(fd, fhdr, isec, shdr) != OK)
			{
				close(fd);
				return(2);
			}
			...
			// section
			if (printlnno(fd, fhdr, isec, shdr) != OK)
			{
				close(fd);
				return(2);
			}
