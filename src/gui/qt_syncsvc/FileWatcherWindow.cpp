...
// ctors and dtor
FileWatcherWindow::FileWatcherWindow(const QString &dasname, DHMWindow *dhmwindow):
    mainboxlayout_(NULL), 
    buttonboxlayout_(NULL), 
    onetextline_(NULL), 
    watchbutton_(NULL),  
    messages_(NULL),
    dhmwindow_(dhmwindow),
    counter_(0),
    watcheddirs_(),
    das_(dasname)
{
    ...
    // connect to main dhm window
    connect(this, SIGNAL(addDAS(const QString &)), dhmwindow_, SLOT(onAddDAS(const QString &)));
    connect(this, SIGNAL(removeDAS(const QString &)), dhmwindow_, SLOT(onRemoveDAS(const QString &)));
    connect(this, SIGNAL(addFileNodeToDAS(const QString &, const QString &)), dhmwindow_, SLOT(onAddFileNodeToDAS(const QString &, const QString &)));
    connect(this, SIGNAL(removeFileNodeToDAS(const QString &, const QString &)), dhmwindow_, SLOT(onRemoveFileNodeToDAS(const QString &, const QString &)));
    ...
    // label window
    setWindowTitle(QString("%1 Viewer").arg(das_.getName()));

    // register DAS with DHM
    emit(addDAS(das_.getName()));
}

FileWatcherWindow::~FileWatcherWindow()
{
    emit(removeDAS(das_.getName()));
}
...
FileWatcherWindow::onWatchButtonClicked()
{
    ...
    emit(addFileNodeToDAS(das_.getName(), filenode));
...
FileWatcherWindow::onWatchedListUnWatchButtonClicked()
{
    ...
    for ( ; sit != sitend; ++sit)
    {
        ...
        emit(removeFileNodeToDAS(das_.getName(), filenode));
