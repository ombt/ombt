...
void
MdiMainWindow::createDHMWindow()
{
    dhmsubwindow_ = new DHMWindow();
    mdiarea_->addSubWindow(dhmsubwindow_);
    dhmsubwindow_->show();
    statusBar()->showMessage(tr("DHM Window created"), 2000);
}
...
{
    static unsigned int dasId = 0;
    FileWatcherWindow *subwindow =
        new FileWatcherWindow(QString("DAS-%1").arg(++dasId), dhmsubwindow_);
