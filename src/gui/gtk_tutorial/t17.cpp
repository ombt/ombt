                                     ...
                                     gint          buff_len )
{
  ...
  gtk_calendar_get_date (GTK_CALENDAR (data->window),
			 &year, &month, &day);
  g_date_set_dmy (&date, day, (GDateMonth)(month + 1), year);
...
static void create_calendar( void )
{
  ...
  struct {
    ...
    {
      { (gchar*)"Show Heading" },
      { (gchar*)"Show Day Names" },
      { (gchar*)"No Month Change" },
      { (gchar*)"Show Week Numbers" },
      { (gchar*)"Week Start Monday" }
    };
