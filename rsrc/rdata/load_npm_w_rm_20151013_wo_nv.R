#
sqlite_load_npm_w_rm_wo_nv <- function(nrows=-1)
{
    ...
    data_Count = 
        sqlite_load_nv_table_from_db(db,"Count",nrows=nrows)
    data_CycleTime = 
        sqlite_load_nv_table_from_db(db,"CycleTime",nrows=nrows)
    data_FID_DATA = 
    ...
    data_Index = 
        sqlite_load_nv_table_from_db(db,"Index",nrows=nrows)
    data_Information = 
        sqlite_load_nv_table_from_db(db,"Information",nrows=nrows)
    data_InspectionData = 
        sqlite_load_nv_table_from_db(db,"InspectionData",nrows=nrows)
    ...
    data_Time = 
        sqlite_load_nv_table_from_db(db,"Time",nrows=nrows)
    #
    sqlite_close_db(db)
    #
