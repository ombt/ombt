...
$csv_base_path = $ENV{'CSV_BASE_PATH'} 
    if (exists($ENV{'CSV_BASE_PATH'}));
$csv_base_path = "." 
    unless (defined($csv_base_path) and ($csv_base_path ne ""));
#
my $csv_rel_path = undef;
$csv_rel_path = $ENV{'CSV_REL_PATH'} 
    if (exists($ENV{'CSV_REL_PATH'}));
$csv_rel_path = "CSV" 
...
sub write_filename_id
{
    ...
    #
    foreach my $key (keys %{$fname_ids{fname_to_fid}})
    {
        printf $outfh "%s%s%s\n", 
            $key, 
            $delimiter,
            $fname_ids{fname_to_fid}{$key};
    }
    #
    close($outfh);
}
