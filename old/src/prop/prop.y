...
ombt::List<ombt::Set_List<Clause> > clausesPrograms;
ombt::BinaryTree_AVL<Clause> clausesDB;
...
dumpClausesPrograms()
{
	...
	// iterate over all programs, and print them.
	ombt::ListIterator<ombt::Set_List<Clause> > cprogsIter(clausesPrograms);
	for (int progno = 1; !cprogsIter.done(); cprogsIter++, progno++)
	{
		// iterate over all clauses in program
		std::cout << std::endl << "CLAUSE PROGRAM: " << progno << std::endl;
		ombt::Set_List_Iterator<Clause> clauseIter(cprogsIter());
...
void
removeTautologies(ombt::Set_List<Clause> &clausesProgram)
{
	Clause clause;
	ombt::Set_List<Clause> newClausesProgram;
	ombt::Set_List_Iterator<Clause> clprogIter(clausesProgram);
	for ( ; !clprogIter.done(); clprogIter++)
	{
		...
		{
			std::cout << "TAUTOLOGY, removing " << clause << std::endl;
		}
	}
...
void
removeSubsumedClauses(ombt::Set_List<Clause> &clausesProgram)
{
	ombt::Set_List<Clause> deletedClauses;
	ombt::Set_List_Iterator<Clause> clprogIter(clausesProgram);
	ombt::Set_List_Iterator<Clause> clprogIter2(clausesProgram);
	for( ; !clprogIter.done(); clprogIter++)
	{
	...
	ombt::Set_List_Iterator<Clause> deletedIter(deletedClauses);
	for( ; !deletedIter.done(); deletedIter++)
	{
		Clause tmp(deletedIter());
		clausesProgram.remove(tmp);
		if (verbose)
		{
			std::cout << "SUBSUMED, removing " << deletedIter() << std::endl;
		}
	}
}
...
void
removeChasteClauses(ombt::Set_List<Clause> &clausesProgram)
{
	...
	for (clprogIter.reset() ; !clprogIter.done(); clprogIter++)
	{
		...
		int includeit;
		for (includeit = 1; !clauseIter.done(); clauseIter++)
		{
		...
		{
			std::cout << "CHASTE, removing " << clause << std::endl;
		}
	}
	clausesProgram = newClausesProgram;
	return;
...
convert2cnf()
{
	...
	ombt::Set_List<Clause> clausesProgram;
	Clause clause;
...
void
dumpClause(const char *searchType, int currentDepth, int icl1, int icl2, 
		int nextClause, Clause &clause)
{
	std::cout << "st(" << searchType << "), ";
	std::cout << "cl(" << nextClause << "), ";
	std::cout << "con(" << clause.isPartOfConclusion() << "), ";
	std::cout << "res(" << icl1 << "," << icl2 << "), ";
	std::cout << "dep(" << currentDepth << ") = ";
...
int
resolveClauses(ombt::Array<ombt::BinaryTree_AVL<Clause> > &clausesArray, 
	const Clause &clause1, const Clause &clause2, int &clausesAdded, int currentDepth)
{
	...
	for ( ; !cl1Iter.done(); cl1Iter++)
	{
		...
		if (clause2.isInClause(negatedAtom))
		{
			...
			// already exists.
			//
			if ((!dbflag || !clausesDB.includes(newcl)) && 
...
int 
linearSearch(ombt::Array<ombt::BinaryTree_AVL<Clause> > &clausesArray, int currentDepth)
{
#if 0
...
int
resolveDepth(ombt::Array<ombt::BinaryTree_AVL<Clause> > &clausesArray, 
	int &clausesAdded, int currentDepth)
{
	// scan over clauses and resolve, when possible
	ombt::BinaryTree_AVL_Iterator_InOrder<Clause>
...
removeSubsumedClauses(
	ombt::Array<ombt::BinaryTree_AVL<Clause> > &clausesArray, int currentDepth)
{
	ombt::Set_List<Clause> deletedClauses;
	for (int idep = 0; idep <= currentDepth; idep++)
	{
...
int
resolveProgram(const ombt::Set_List<Clause> &clauses)
{
	// create an array of trees to track clauses at each depth
	int currentDepth = 0;
	ombt::Array<ombt::BinaryTree_AVL<Clause> > clausesArray(MaxDepth);

	// copy over initial set of clauses
	ombt::Set_List_Iterator<Clause> clausesIter(clauses);
...
resolvePrograms()
{
	...
	if (clausesPrograms.isEmpty())
	{
		std::cout << "No programs to resolve. Returning." << std::endl;
		return;
	}
	...
	for (int progno = 1; !cprogsIter.done(); cprogsIter++, progno++)
	{
		std::cout << std::endl << "RESOLVING CLAUSE PROGRAM: " << progno << std::endl;
		if (resolveProgram(cprogsIter()) == OK)
			std::cout << "PROGRAM " << progno << " SUCCEEDED." << std::endl;
		else
			std::cout << "PROGRAM " << progno << " FAILED." << std::endl;
		clausesDB.clear();
	}
	return;
}
...
{
	std::cout << std::endl;
	std::cout << "usage: " << cmd << " [-?V] [-A [on|off]]" << std::endl;
	std::cout << "	[-C[on|off]] [-D[on|off] " << std::endl;
	std::cout << "	[-S[on|off]] [-T[on|off]] " << std::endl;
	std::cout << "	[-d maxlevel] [-b maxlevel] " << std::endl;
	std::cout << "	input_file ..." << std::endl;
	std::cout << std::endl;
	std::cout << "-V = turn verbose on," << std::endl;
	std::cout << "-A = turn ALL filtering on/off," << std::endl;
	std::cout << "-C = turn chaste clause filtering on/off," << std::endl;
	std::cout << "-D = turn database filtering on/off," << std::endl;
	std::cout << "-S = turn set-of-support filtering on/off," << std::endl;
	std::cout << "-T = turn tautology filtering on/off," << std::endl;
	std::cout << "-U = turn subsumption filtering on/off," << std::endl;
	std::cout << "-d = depth-first levels to search (not implemented)," << std::endl;
	std::cout << "-b = breadth-first levels to search," << std::endl;
	std::cout << std::endl;
	std::cout << "input file contains the clauses to prove." << std::endl;
	std::cout << "by default, the flags -A,-C,-D,-S,-T,-U are all on." << std::endl;
}
...
main(int argc, char **argv)
{
	...
	for (int c = 0; (c = getopt(argc, argv, "?Vb:d:C:D:S:T:U:A:")) != EOF; )
	{
		...
		case 'A':
			if (ombt::String(optarg) == ombt::String("on"))
			{
				allflag = 1;
				supportsetflag = 1;
				tautologyflag = 1;
			...
			else if (ombt::String(optarg) == ombt::String("off"))
			{
				allflag = 0;
				supportsetflag = 0;
				tautologyflag = 0;
