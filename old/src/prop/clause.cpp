...
Clause::Clause(const ombt::Set_List<Atom> &alist, int d, int poc):
...
Clause::operator<(const Clause &rhs) const
{
	ombt::Set_List_Iterator<Atom> ai(atoms);
	ombt::Set_List_Iterator<Atom> rhsai(rhs.atoms);
...
{
	ombt::Set_List_Iterator<Atom> atomsIter(atoms);
...
LClause::operator==(const Clause &rhs) const
{
	...
	for (thislen = 0; !thisIter.done(); thislen++, thisIter++);
	ClauseIterator rhsIter(rhs);
	for (rhslen = 0; !rhsIter.done(); rhslen++, rhsIter++);
