...
CoinsNode::heuristic(const CoinsNode &start)
{
	...
	for (int ib = 0; ib < number; ib++)
	{
		...
		case 'P':
			break;
		case 'F':	
			hvalue += (offset+number-ib)*(offset+number-ib);
			offset++;
			break;
		}
	}
	...
	for ( ; ib <= 2*number; ib++)
	{
		...
		{
		case 'P':
			hvalue += (offset+ib-number)*(offset+ib-number);
			offset++;
			break;
		case 'F':	
			break;
	...
	// counters for getting h-value estimate
	Array<int> oopfs(number+1);
	Array<int> oopps(number+1);

	// find locations of out-place coins
	int oopfsi = -1;
	for (int ib = number; ib >= 0; ib--)
	{
		if (board[ib] == 'F' || board[ib] == ' ')
			oopfs[++oopfsi] = ib;
	}
	int ooppsi = -1;
	for (ib = number; ib <= 2*number; ib++)
	{
		if (board[ib] == 'P' || board[ib] == ' ')
			oopps[++ooppsi] = ib;
	}

	// start calculating
	int i = 0;
	for (ib = number; ib >= 0; ib--)
	{
		if (board[ib] == 'F')
		{
			hvalue += (oopps[i]-ib)*(oopps[i]-ib);
			i++;
		}
	}
	i = 0;
