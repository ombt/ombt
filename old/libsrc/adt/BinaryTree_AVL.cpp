...
{
	int heightChanged;
	return(insert(root, data, heightChanged));
}

template <class DataType>
int
BinaryTree_AVL<DataType>::update(const DataType &data)
{
	if (includes(data))
		return(insert(data));
	else
...
template <class DataType>
int
BinaryTree_AVL<DataType>::includes(const DataType &data) const
{
...
template <class DataType>
std::ostream &
BinaryTree_AVL<DataType>::dump(std::ostream &os) const
{
	dump(root, os);
	return(os);
}
...
template <class DataType>
int
BinaryTree_AVL<DataType>::includes(const BinaryTreeNode_AVL<DataType> *node, 
			     ...
			     const DataType &data, int &heightChanged)
{
	...
	else if (data < btn->data)
	{
		...
		if (insert(btn->left, data, heightChanged) != OK)
			return(NOTOK);
	...
	else if (data > btn->data)
	{
		...
		if (insert(btn->right, data, heightChanged) != OK)
			return(NOTOK);
...
BinaryTree_AVL<DataType>::audit(const BinaryTreeNode_AVL<DataType> *btn) const
{
	...
	else
	{
		...
		{
			std::cout << "<" << btn->data << "," << btn->balance << ",";
			std::cout << left << "," << right << ">" << std::endl;
			std::cout << "MISMATCH FOUND !!!" << std::endl;
			std::cout << "bt is ... " << *this << std::endl;
		}
		return(height);
	}
}
...
{
	this->stack.clear();
	for (BinaryTreeNode_AVL<DataType> *pn = this->tree->root; 
		pn != NULL; pn = pn->left)
	{
		this->stack.push(pn);
	}
