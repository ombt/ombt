...
//
VECTOR_$TYPE fexample(const VECTOR_$TYPE A, $TYPE s)
{
    int N = A.size();
    VECTOR_$TYPE B(N);
...
int TestVec_$TYPE(int N, int start, int End)
{

    int i;

    cout << endl;
    cout << "TestVec_$TYPE :" << endl;
    ...
    cout << "Testing VECTOR_$TYPE &c = C(I)" << endl;
...
// result and hence not compilable on all C++ platforms.
    ...
    cout <<  " VECTOR_$TYPE E(&F(0),N) " << endl;

    VECTOR_$TYPE E(&F[0], N);
    cout << "E : " << endl << E << endl;
    ...
    cout <<  " VECTOR_$TYPE G(&F(0),N, MV_Vector_::ref) " << endl;

    VECTOR_$TYPE G(&F[0], N, MV_Vector_::ref);
    ...
    cout << "Testing returning subvectors from functions:  " 
        << endl << "   VECTOR_$TYPE K = fexample(A,1.0); " << endl;
    //
    VECTOR_$TYPE K = fexample(A,1);
    //
    cout << "A" << endl << A << endl;
    cout << "K" << endl << K << endl;


    VECTOR_$TYPE Q( &A(0), A.size(), MV_Vector_::ref);
    A() = B;
    //
