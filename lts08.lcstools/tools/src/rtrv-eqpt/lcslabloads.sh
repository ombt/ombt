...
usage: ${0} [-?] [labid] [labid ...]
or
usage: ${0} [-?] -c cpuload [cpuload ...]
or
usage: ${0} [-?] -b branch [branch ...]
or
usage: ${0} [-?] [-q | -l]
"
	exit 0
...
then
	...
	for cpuload in ${*}
	do
		uprint -h "Labs available for reload using load ${cpuload}:" labloads where cpuload req "^${cpuload}$"
	done
elif [[ ${1} == "-b" ]]
then
	cd ${LCSTOOLSDATA}
	...
	for branch in ${*}
	do
		uprint -h "Labs available for reload using branch ${branch}:" labloads where branch req "^${branch}$"
	done
elif [[ ${1} == "-l" ]]
then
	cd ${LCSTOOLSDATA}
	uprint -h "Loads available for reload:" labloads 
elif [[ ${1} == "-q" ]]
then
	cd ${LCSTOOLSDATA}
	#
	PS3="Choose labid: "
	select labid in QUIT $(uprintf -q -f"%s\n" labid in chassis | sort -u)
	do
		case "${labid}" in
		QUIT)
			exit 0
