...
#
proc remote_ls { branch machine directory ispatch outfile } {
	#
	global env;
	global username;
	global userpasswd;
...
#
proc parse_local_file { ispatch localfile imagelistname } {
...
proc getimages { branch imagelistname } {
	...
	set ispatch [lindex $bdata 3];
	#
	switch -regexp -- $ispatch {
	{^[yY][eE][sS]} {
		set ispatch 1;
		puts "\nPatch flag is ON";
	}
	...
	if {[catch { 
		...
		remote_ls $branch $machine $directory $ispatch remotefile;
		...
		parse_local_file $ispatch $localfile $imagelistname
		# remove local file
...
proc saveload { } {
	...
	global spawn_id;
	#
	if {[dbselect branches buildmachines "" "branch machine directory patch"] != 0} {
