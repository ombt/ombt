...
insert ${variables} "TCLBIN" '$(HOME)/bin'
...
function writetargets() {
	...
	for (target in targets)
	{
		...
		else if (targettype[target] == "TCLSCRIPT")
		{
			printf("\n%s:", target);
			#
			...
			printf("\t\t$(ENDLIST)\n");
			#
			printf("\t\t-rm -f $(TCLBIN)/%s\n", target);
			printf("\t\t-cp %s %s\n", targetlist[1], target);
			printf("\t\tchmod +x %s\n", target);
			printf("\t\tcp %s $(TCLBIN)/%s\n", target, target);
		}
...
function writemymk() {
	trace("writemymk 10 ", "entry ...");
	#
	# write mymk for targets.
	#
	printf("\n");
	printf("mymk:\n");
	#
