...
public: // data types and constants
    ...
    // store allowed knight-moves for a board
    class AllowedMoves
    {
    public:
        // ctor and dtor
        AllowedMoves(int key_num):
            _key_num(key_num),
            _imax(0),
            _moves(NULL)
        {
            MustBeTrue(_key_num > 0);
    ...
    private:
        // not needed
        AllowedMoves();
        AllowedMoves(const AllowedMoves &);
        AllowedMoves &operator=(const AllowedMoves &);
