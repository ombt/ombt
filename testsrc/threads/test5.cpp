...
main(int argc, char **argv)
{
    ...
    std::list<ThreadData<SimpleThread> > readerthreads;
	...
	readerthreads.push_back(ThreadData<SimpleThread>(ireader, st));
    }

    ThreadBarrier tb(true);

    std::list<UseCntPtr<Writer> > writers;
    std::list<ThreadData<SimpleThread> > writerthreads;
	...
	writerthreads.push_back(ThreadData<SimpleThread>(iwriter, st));
    }
    ...
    std::list<ThreadData<SimpleThread> >::iterator writer = writerthreads.begin();
    std::list<ThreadData<SimpleThread> >::iterator writerend = writerthreads.end();
    ...
    std::list<ThreadData<SimpleThread> >::iterator reader = readerthreads.begin();
    std::list<ThreadData<SimpleThread> >::iterator readerend = readerthreads.end();
