...
std::list<UseCntPtr<Thread> > acceptthreads;
...
public:
    ...
    virtual int run()
    {
        ...
        while (clientsleft)
        {
            ...
            acceptthreads.push_back(pserverthread);
...
main(int argc, char **argv)
{
    ...
    DBGTRACE();
    std::list<UseCntPtr<Thread> >::iterator ait = acceptthreads.begin();
    std::list<UseCntPtr<Thread> >::iterator aitend = acceptthreads.end();
    DBGTRACE();
    for ( ; ait != aitend; ++ait)
    {
        DBGTRACE();
        MustBeTrue((*ait)->join(retval) == 0);
        DBGTRACE();
    }
    DBGTRACE();
