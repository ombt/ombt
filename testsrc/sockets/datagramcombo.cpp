...
public:
    // ctor and dtor
    EchoClient(const std::string &host, const std::string &path,
               const std::string &serverpath, ThreadBarrier* ptb,
               int msgs_to_send):
        Task(), host_(host), path_(path), serverpath_(serverpath),
...
public:
    // ctor and dtor
    EchoServer(const std::string &host, const std::string &path, 
               ThreadBarrier *ptb, int msgs_to_read):
        Task(), host_(host), path_(path), ptb_(ptb), msgs_to_read_(msgs_to_read)
    {
        DBGTRACE();
    }
    virtual ~EchoServer()
...
main(int argc, char **argv)
{
    ...
    std::string clientpath("/tmp/client" + ss.str());

    DBGPRINTF("Host       : %s",  host.c_str());
    DBGPRINTF("Server Path: %s",  serverpath.c_str());
    DBGPRINTF("Client Path: %s",  clientpath.c_str());

    UseCntPtr<ThreadBarrier> ptb(new ThreadBarrier(true));

    UseCntPtr<EchoServer> pservertask(new EchoServer(host, serverpath,
    ...
    UseCntPtr<EchoClient> pclienttask(new EchoClient(host, clientpath, 
                                      serverpath, ptb, numberOfMsgsToSend));
