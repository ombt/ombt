...
main(int argc, char **argv)
{
    ...
    dbgprintf("Create a Notify socket ...\n");
    UseCntPtr<Notify> pnotifier(new Notify);
    MustBeTrue(pnotifier->isOk());

    for (++arg; arg<argc; ++arg)
    {
        pnotifier->watch(argv[arg], IN_CLOSE_WRITE|IN_MODIFY|IN_CREATE|IN_DELETE);
    }

    dbgprintf("Create an Notifier EndPoint ...\n");
    UseCntPtr<NotifyEndPoint> pendpoint(new NotifyEndPoint(pnotifier));
    MustBeTrue(pendpoint->isOk());

    dbgprintf("Create Notify Handler ...\n");
    ...
    MustBeTrue(preactor->registerReadHandler(pendpoint, pbioh) == OK);
